...
public class HomePageCallback extends TemplatePageCallback
{
  ...
  {
    String out = page.replaceAll("\\{%TITLE%\\}",
        Matcher.quoteReplacement(htmlEscape(m_lab.getTitle())));
  ...
  /**
   * Replaces all the bits of text of the form <tt>{J xyz J}</tt> by a call to the
   * static method <tt>xyz.getClassText()</tt>. If class xyz does not exist or
   * static method <tt>getClassText()</tt> does not exist, the empty string is
