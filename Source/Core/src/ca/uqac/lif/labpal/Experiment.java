...
import ca.uqac.lif.labpal.provenance.ExperimentProvenanceCellLeaf;
import ca.uqac.lif.labpal.provenance.ExperimentProvenanceLeaf;
...
public abstract class Experiment implements Runnable, DataOwner
{
	...
	@Override
	public final ExperimentProvenanceLeaf dependsOn(String id)
	{
		// Points generated by an experiment do not depend on anything else
		return new ExperimentProvenanceLeaf(id, this);
	}
	
	public final ExperimentProvenanceLeaf dependsOn(String id, int position)
	{
		// Points generated by an experiment do not depend on anything else
		return new ExperimentProvenanceCellLeaf(id, position, this);
	}
	
	/**
	 * Gets the data point ID associated to an experiment's parameter
	 * @param key The key
	 * @return The ID, or -1 if key does not exist
	 */
	public final String getDataPointId(String key)
	{
		return "E" + getId() + ":" + key;
	}
